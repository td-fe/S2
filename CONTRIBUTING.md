...
4. `yarn core:start` 可视化的方式调试核心层 `@tant/s2` (基于 jest-electron)
5. `yarn react:start` 可视化的方式调试组件层 `@tant/s2-react` (基于 jest-electron)
6. `yarn vue:start` 可视化的方式调试组件层 `@tant/s2-vue` (基于 jest-electron)
7. `yarn react:playground` 启动本地的组件层 `@tant/s2-react` demo (基于 vite)
8. `yarn vue:playground` 启动本地的组件层 `@tant/s2-vue` demo (基于 vite)
9. `yarn build` 构建 `@tant/s2` 和 `@tant/s2-react` 和 `@tant/s2-vue` 三个包, 分别输出 `umd`, `esm` 和 `lib` 目录
